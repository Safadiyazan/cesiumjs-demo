...
description: LAATSim is a simulation tool for Advanced Air Mobility (AAM) systems. It replicates AAM systems in a digital environment, allowing users to model and analyze their behavior and performance accurately. LAATSim sets the stage for groundbreaking research in the AAM field and aims to inspire collaboration for innovation and advancement.
---
...
The LAATSim is a powerful web-based application designed to streamline and enhance air mobility scenario modeling, analysis, and optimization. This modular tool leverages the strengths of various technologies to deliver a user-friendly and visually immersive experience. CesiumJS is a powerful open-source JavaScript library specifically designed for creating interactive 3D globes and maps within web browsers. It leverages WebGL for hardware-accelerated graphics, enabling smooth and performant visualization of geospatial data. Let's delve into how CesiumJS plays a crucial role in this application:
...
To seamlessly visualize simulation results in CesiumJS, a robust mechanism synchronizes data between MATLAB and the web-based frontend, as follow:
...
for i = 1:M
    ...
    stat = double(full_stat(i, :));  % aircraft flight staus {0-inactive, 1-active, 2-landed}
...
* **Entity Creation:** For each position in the trajectory, a corresponding entity is created or updated within the `entitiesArray`. This entity holds the aircraft's visual representation and its associated geospatial coordinates at that specific time step.
...
#### (iii) Visualizing Airspace Structure

This section describes the code responsible for visualizing airspace structure within the CesiumJS project. CesiumJS is a powerful open-source JavaScript library specifically designed for creating interactive 3D globes and maps within web browsers. It leverages WebGL for hardware-accelerated graphics, enabling smooth and performant visualization of geospatial data.
...
<div align="center">
**Code 6.** *Visualizing Airspace Structure [javascript]*
</div>
```javascript
...
- The initial camera position provides a specific viewpoint of the airspace. CesiumJS offers extensive camera controls for users to freely explore the 3D globe.
...
**Figure 4.** *Aircraft structure visualized in CesiumJS viewer with `Center=(-73.984359, 40.751718,480)`*
</div>

<br />
<div align="center">
![Screenshot of LAATSim](./figures/LAATSimAirspaceZurich.png)
**Figure 5.** *Aircraft structure visualized in CesiumJS viewer with `Center=(8.545094,47.373878,480)`*
</div>

<br />
<div align="center">
![Screenshot of LAATSim](./figures/LAATSimAirspaceDubai.png)
**Figure 6.** *Aircraft structure visualized in CesiumJS viewer with `Center=(55.216305,25.196254,480)`*
</div>

<br />
#### (iv) Interactive Navigation within Airspace Using Keyboard Controls
...
* **`onKeyDown`**: This function registers a key press event and performs the following actions:
    ...
    * Attempts to focus on a random available aircraft when the 'r' key is pressed.
    * Zooms to a random available aircraft when the 'f' key is pressed.
* **`onKeyUp`**: This function registers a key release event and updates the `keyState` object to reflect the released key.
* **`handleMovement`**: This function continuously updates the camera's orientation and movement based on the currently pressed keys. It performs the following actions:
    * Checks the `NavigationOn` flag to ensure keyboard navigation is enabled before proceeding.
...
* Modifying the keyboard shortcuts to better suit your user preferences.
* Implementing additional navigation controls, such as mouse-based controls or a virtual joystick.
...
<div align="center">
**Code 7.** *Navigation Keyboard Setting [javascript]*
</div>
``` javascript
...
<br />
#### (v) User Interface and React Capabilities

**User Interface and React Capabilities** The user interface (UI) plays a crucial role in this project, providing an interactive environment for users to configure and run simulations. Built with React, the UI offers several key benefits:

- **User-Defined Parameters:** Users have the ability to control various simulation parameters through the UI. This includes:
    - *Airspace Size*: Specify the dimensions of the virtual airspace where the aircraft will operate.
    - *Maximum Aircraft Speed*: Define the upper limit for the speed of aircraft within the simulation.
...
- **Dynamic Simulation Control:** Users can adjust these parameters directly in the UI, allowing for on-the-fly modification and exploration of different scenarios. As these user inputs change, the UI sends the updated configuration to the server.
...
- **CesiumJS Visualization:** Once the simulation is complete, the results are visualized within the CesiumJS environment. This allows users to observe the simulated air traffic patterns and analyze the impact of their configuration choices.
- **Modular Design with React:** React's component-based architecture makes the UI modular and extensible. This facilitates future enhancements, such as adding new configuration options or integrating with different visualization tools.
...
<br />
### Conclusion
This project demonstrates a powerful framework for simulating and visualizing air traffic scenarios using CesiumJS and a user-friendly React-based UI. By enabling users to define key parameters like airspace size, aircraft speed, safety radius, and demand, the UI facilitates exploration of diverse air traffic conditions. The modular design of the React UI allows for future enhancements, paving the way for a more comprehensive simulation environment.
...
- **Enhanced User Controls:**
    ...
    - **Advanced Input Features:** Explore the use of sliders, dropdowns, or input fields for more granular control over simulation parameters.
